...
'''
class refbox_naviSM(Behavior):
	'''
	Combined RoboCup@Work referee box and navigation behaviour.
	'''


	def __init__(self):
		super(refbox_naviSM, self).__init__()
		self.name = 'refbox_navi'
	...
	def create(self):
		...
		with _state_machine:
			...
			OperatableStateMachine.add('move_base',
										MoveBaseState(),
										transitions={'arrived': 'base_placement', 'failed': 'refbox_planner'},
			...
			# x:681 y:310
			OperatableStateMachine.add('base_placement',
										ApproachServiceAreaState(),
										transitions={'success': 'refbox_planner', 'failed': 'refbox_planner'},
